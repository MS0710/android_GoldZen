...
public class Game8Activity extends AppCompatActivity {
    private String TAG = "Game8Activity";
    private ImageView img_game8_issue,img_game8_next;
    private CardView cv_game8_ansA,cv_game8_ansB,cv_game8_ansC,cv_game8_ansD;
    private ImageView img_game8_ansA,img_game8_ansB,img_game8_ansC,img_game8_ansD;
    private TextView txt_game8_timeCunt;
    ...
    private void initView(){
        ...
        mediaPlayer = MediaPlayer.create(Game8Activity.this, R.raw.back3);
        initSoundPool();

        txt_game8_timeCunt = (TextView)findViewById(R.id.txt_game8_timeCunt);
        img_game8_ansA = (ImageView) findViewById(R.id.img_game8_ansA);
        img_game8_ansB = (ImageView) findViewById(R.id.img_game8_ansB);
        img_game8_ansC = (ImageView) findViewById(R.id.img_game8_ansC);
        img_game8_ansD = (ImageView) findViewById(R.id.img_game8_ansD);
        img_game8_issue = (ImageView) findViewById(R.id.img_game8_issue);
        cv_game8_ansA = (CardView) findViewById(R.id.cv_game8_ansA);
        cv_game8_ansB = (CardView) findViewById(R.id.cv_game8_ansB);
        cv_game8_ansC = (CardView) findViewById(R.id.cv_game8_ansC);
        cv_game8_ansD = (CardView) findViewById(R.id.cv_game8_ansD);
        img_game8_next = (ImageView) findViewById(R.id.img_game8_next);
        cv_game8_ansA.setOnClickListener(onClick);
        cv_game8_ansB.setOnClickListener(onClick);
        cv_game8_ansC.setOnClickListener(onClick);
        cv_game8_ansD.setOnClickListener(onClick);
        img_game8_next.setOnClickListener(onClick);
    ...
    private View.OnClickListener onClick = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            playSound(1, 0);
            if(view.getId() == R.id.cv_game8_ansA){
                item = 0;
                if (flag_ans[item]){
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#FBFDEF"));
                    cv_game8_ansA.setCardBackgroundColor(colorStateList);
                    img_game8_ansA.setVisibility(View.GONE);
                    flag_ans[item] = false;
                    dressUp();
                }else {
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#F5BACA"));
                    cv_game8_ansA.setCardBackgroundColor(colorStateList);
                    img_game8_ansA.setVisibility(View.VISIBLE);
                    flag_ans[item] = true;
                    dressUp();
                }
            }else if(view.getId() == R.id.cv_game8_ansB){
                item = 1;
                if (flag_ans[item]){
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#FBFDEF"));
                    cv_game8_ansB.setCardBackgroundColor(colorStateList);
                    img_game8_ansB.setVisibility(View.GONE);
                    flag_ans[item] = false;
                    dressUp();
                }else {
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#F5BACA"));
                    cv_game8_ansB.setCardBackgroundColor(colorStateList);
                    img_game8_ansB.setVisibility(View.VISIBLE);
                    flag_ans[item] = true;
                    dressUp();
                }
            }else if(view.getId() == R.id.cv_game8_ansC){
                item = 2;
                if (flag_ans[item]){
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#FBFDEF"));
                    cv_game8_ansC.setCardBackgroundColor(colorStateList);
                    img_game8_ansC.setVisibility(View.GONE);
                    flag_ans[item] = false;
                    dressUp();
                }else {
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#F5BACA"));
                    cv_game8_ansC.setCardBackgroundColor(colorStateList);
                    img_game8_ansC.setVisibility(View.VISIBLE);
                    flag_ans[item] = true;
                    dressUp();
                }
            }else if(view.getId() == R.id.cv_game8_ansD){
                item = 3;
                if (flag_ans[item]){
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#FBFDEF"));
                    cv_game8_ansD.setCardBackgroundColor(colorStateList);
                    img_game8_ansD.setVisibility(View.GONE);
                    flag_ans[item] = false;
                    dressUp();
                }else {
                    colorStateList = ColorStateList.valueOf(Color.parseColor("#F5BACA"));
                    cv_game8_ansD.setCardBackgroundColor(colorStateList);
                    img_game8_ansD.setVisibility(View.VISIBLE);
                    flag_ans[item] = true;
                    dressUp();
                }
            }else if(view.getId() == R.id.img_game8_next){
                if(checkIssue()){
                    if (aHandler != null) {
    ...
    private void showPassAlertDialog(){
        playSound(3, 0);//Pass
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(Game8Activity.this);
    ...
    final Runnable runnable = new Runnable() {
        public void run() {
            ...
            if (count > 0) {
                txt_game8_timeCunt.setText(Integer.toString(count));
                count--;
                aHandler.postDelayed(runnable, 1000);
            }else{
                Log.d(TAG, "run: time up");
                txt_game8_timeCunt.setText("0");
                spendTime = 20;
    ...
    private void resetAll(){
        ...
        cv_game8_ansC.setCardBackgroundColor(colorStateList);
        cv_game8_ansD.setCardBackgroundColor(colorStateList);
        img_game8_issue.setImageResource(R.drawable.ans4_ori);
    ...
    private void dressUp(){
        if(flag_ans[0] && !flag_ans[1] && !flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_g);
        }else if(!flag_ans[0] && flag_ans[1] && !flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_c);
        }else if(!flag_ans[0] && !flag_ans[1] && flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm);
        }else if(!flag_ans[0] && !flag_ans[1] && !flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_f);
        }else if(flag_ans[0] && flag_ans[1] && !flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_c);
        }else if(flag_ans[0] && !flag_ans[1] && flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_g);
        }else if(flag_ans[0] && !flag_ans[1] && !flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_g_f);
        }
        //////
        else if(!flag_ans[0] && flag_ans[1] && flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_c);
        }else if(!flag_ans[0] && flag_ans[1] && !flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_c_f);
        }
        //////
        else if(!flag_ans[0] && !flag_ans[1] && flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_f);
        }
        //////
        else if(flag_ans[0] && flag_ans[1] && flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_c_g);
        }else if(flag_ans[0] && flag_ans[1] && !flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_c_g_f);
        }else if(flag_ans[0] && !flag_ans[1] && flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_g_f);
        }
        //////
        else if(!flag_ans[0] && flag_ans[1] && flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_c_f);
        }
        //////
        else if(flag_ans[0] && flag_ans[1] && flag_ans[2] && flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_sm_c_g_f);
        }
        //////
        else if(!flag_ans[0] && !flag_ans[1] && !flag_ans[2] && !flag_ans[3]){
            img_game8_issue.setImageResource(R.drawable.ans4_ori);
